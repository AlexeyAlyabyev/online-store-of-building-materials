...
class ControllerCheckoutCart extends Controller {
	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['total_short'] = $this->cart->countProducts();
    ...
    // ---- Пересчет всех сумм на станице
      $json['total_short'] = $this->cart->countProducts();
      if (($json["total_short"] % 10) == 1 && ($json["total_short"] < 10 || $json["total_short"] > 20))
				$json["total_short_text"] = $json['total_short']." товар";
			else if ((($json["total_short"] % 10) >= 2 && ($json["total_short"] % 10) < 5) && ($json["total_short"] < 10 || $json["total_short"] > 20))
				$json["total_short_text"] = $json['total_short']." товара";
			else
				$json["total_short_text"] = $json['total_short']." товаров";
      $products_data = $this->cart->getProducts();
      // print_r($products_data);
      $json['products'] = [];
      foreach ($products_data as $product){
        $json['products'][$product['cart_id']]['total'] = $product['total'];
        $json['products'][$product['cart_id']]['discount'] = $product['discount'];
      }
      $json["initial_total"] = $this->cart->getInitialSubTotal(); // общая цена без скидки
      $json["total"] = $this->cart->getSubTotal(); // общая цена со скидкой
      $json["discount"] = $json["initial_total"] - $json["total"];
      // ----
		}
