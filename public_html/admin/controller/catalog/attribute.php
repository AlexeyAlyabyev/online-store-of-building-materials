...
class ControllerCatalogAttribute extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->get['attribute_id'])) {
			$attribute_data = $this->model_catalog_attribute->getAttribute($this->request->get['attribute_id']);
			$data['list_type'] = $attribute_data['list_type'];
			$data['list_values'] = $this->model_catalog_attribute->getAttributeValues($this->request->get['attribute_id']);
		}
		else {
			$data['list_type'] = false;
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				$attribute_data = $this->model_catalog_attribute->getAttribute($result['attribute_id']);
				$json[] = array(
					'attribute_id'    => $result['attribute_id'],
					'name'            => strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),
					'attribute_group' => $result['attribute_group'],
					'list_type' => $attribute_data['list_type'],
					'list_values' => $this->model_catalog_attribute->getAttributeValues($result['attribute_id'])
				);
			}
		}
